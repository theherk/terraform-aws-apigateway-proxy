...
| <a name="input_health_check_id"></a> [health\_check\_id](#input\_health\_check\_id) | ID of the health check to associate with the route53 record. Only applicable if `domain_name` and `zone_id` are given. This is useful if you want to use Route53 health checks to failover to an alternate API. | `string` | `null` | no |
